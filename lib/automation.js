...
async function runReports({ excelPath, year, month, outputMode, outputDir, username, password, onProgress, userDataDir, cacheDir }) {
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      // Enable the civil-ID column (إعدادات الأعمدة → الرقم المدني → تطبيق)
      await page.evaluate(() => { const b = document.getElementById('column-setting'); if (b) b.click(); });
      // wait for the modal's checkboxes to actually render
      try { await page.waitForSelector('input[value=civil_id]', { visible: true, timeout: 8000 }); } catch (e) {}
      await new Promise(r => setTimeout(r, 400));
      const civilColOn = await page.evaluate(() => {
        const cb = document.querySelector('input[value=civil_id]');
        if (!cb) return false;
        if (!cb.checked) cb.click();
        const ok = cb.checked;
        // click the VISIBLE تطبيق button (the one inside the open modal), not a hidden one
        const ap = [...document.querySelectorAll('button')].find(x => x.offsetParent !== null && x.textContent.trim() === 'تطبيق');
        if (ap) ap.click();
        return ok;
      });
      if (!civilColOn) onProgress('   ⚠ تعذّر تفعيل عمود الرقم المدني — تحقّق من إعدادات الأعمدة.');
